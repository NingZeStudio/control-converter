...
pub const FCL_CONTROLLER_VERSION: i64 = 21;
pub const ZL_EDITOR_VERSION: i64 = 12;
pub const META_KEY: &str = "_control_byIQge报错别找我";
pub const META_SCHEMA_VERSION: i64 = 1;

/// ZLKeyAliases maps various key aliases to canonical GLFW names.
pub static ZL_KEY_ALIASES: LazyLock<HashMap<&'static str, &'static str>> = LazyLock::new(|| {
    ...
    for i in 0..10 {
        let c = (b'0' + i) as char;
        let key: String = format!("key.keyboard.{}", c);
        let val: String = format!("GLFW_KEY_{}", c);
        m.insert(Box::leak(key.into_boxed_str()), Box::leak(val.into_boxed_str()));
    }
    for code in b'a'..=b'z' {
        let key: String = format!("key.keyboard.{}", code as char);
        let val: String = format!("GLFW_KEY_{}", (code - 32) as char);
        m.insert(Box::leak(key.into_boxed_str()), Box::leak(val.into_boxed_str()));
    }
    for i in 1..=25 {
        let key: String = format!("key.keyboard.f{}", i);
        let val: String = format!("GLFW_KEY_F{}", i);
        m.insert(Box::leak(key.into_boxed_str()), Box::leak(val.into_boxed_str()));
    }
    m
});

pub static FCL_MOUSE: LazyLock<HashMap<&'static str, i64>> = LazyLock::new(|| {
    HashMap::from([
...
pub static FCL_MOUSE_REVERSE: LazyLock<HashMap<i64, &'static str>> = LazyLock::new(|| {
    HashMap::from([
...
pub static FCL_SCROLL_REVERSE: LazyLock<HashMap<i64, (&'static str, &'static str)>> =
    LazyLock::new(|| {
        HashMap::from([
            (
                1003,
                ("launcher.event.scroll_up.single", "launcher.event.scroll_up"),
            ),
            (
                1004,
                (
                    "launcher.event.scroll_down.single",
                    "launcher.event.scroll_down",
                ),
            ),
        ])
    });

pub static ZL_ONLY_KEYS: LazyLock<HashSet<&'static str>> = LazyLock::new(|| {
    HashSet::from([
        "GLFW_KEY_WORLD_1",
        "GLFW_KEY_WORLD_2",
        "GLFW_KEY_F25",
...
pub static UNSUPPORTED_ZL_KEY_REASONS: LazyLock<HashMap<&'static str, &'static str>> =
    LazyLock::new(|| {
        ZL_ONLY_KEYS
            .iter()
            .map(|k| {
                (
                    *k,
                    "FCL controls do not define an exact matching keycode",
                )
            })
            .collect()
    });

pub static UNSUPPORTED_FCL_KEY_REASONS: LazyLock<HashMap<i64, &'static str>> =
    LazyLock::new(|| {
        HashMap::from([
...
pub static GLFW_TO_FCL: LazyLock<HashMap<&'static str, i64>> = LazyLock::new(|| {
    HashMap::from([
...
pub static FCL_TO_GLFW: LazyLock<HashMap<i64, &'static str>> = LazyLock::new(|| {
    GLFW_TO_FCL.iter().map(|(k, v)| (*v, *k)).collect()
});

/// ZLToFCLFallbacks maps unsupported ZL keys to (fcl_keycode, reason).
pub struct ZlToFclFallback {
    pub keycode: i64,
    pub reason: &'static str,
}

pub static ZL_TO_FCL_FALLBACKS: LazyLock<HashMap<&'static str, ZlToFclFallback>> =
    LazyLock::new(|| {
        let g = |k: &str| GLFW_TO_FCL[k];
        let items: Vec<(&str, i64, &str)> = vec![
            ...
            ("GLFW_KEY_F25", g("GLFW_KEY_F24"), "FCL has no F25; substituted with F24"),
            ("GLFW_KEY_WORLD_1", g("GLFW_KEY_UNKNOWN"), "FCL has no WORLD_1; substituted with UNKNOWN"),
            ("GLFW_KEY_WORLD_2", g("GLFW_KEY_UNKNOWN"), "FCL has no WORLD_2; substituted with UNKNOWN"),
            ("GLFW_KEY_MENU", g("GLFW_KEY_UNKNOWN"), "FCL has no menu key; substituted with UNKNOWN"),
            ("GLFW_KEY_LAST", g("GLFW_KEY_UNKNOWN"), "FCL has no LAST sentinel key; substituted with UNKNOWN"),
            ("GLFW_MOD_SHIFT", g("GLFW_KEY_LEFT_SHIFT"), "FCL has no modifier event; substituted with left shift"),
            ("GLFW_MOD_CONTROL", g("GLFW_KEY_LEFT_CONTROL"), "FCL has no modifier event; substituted with left control"),
            ("GLFW_MOD_ALT", g("GLFW_KEY_LEFT_ALT"), "FCL has no modifier event; substituted with left alt"),
            ("GLFW_MOD_SUPER", g("GLFW_KEY_LEFT_SUPER"), "FCL has no modifier event; substituted with left super"),
            ("GLFW_MOD_CAPS_LOCK", g("GLFW_KEY_CAPS_LOCK"), "FCL has no modifier event; substituted with caps lock"),
            ("GLFW_MOD_NUM_LOCK", g("GLFW_KEY_NUM_LOCK"), "FCL has no modifier event; substituted with num lock"),
            ("GLFW_MOUSE_BUTTON_LAST", 1004, "FCL has no LAST mouse sentinel; substituted with scroll down"),
        ];
        items
            .into_iter()
            .map(|(k, keycode, reason)| {
                (
                    k,
                    ZlToFclFallback {
                        keycode,
...
pub struct FclToZlFallback {
    pub event_type: &'static str,
    pub key: &'static str,
    pub reason: &'static str,
}

pub static FCL_TO_ZL_FALLBACKS: LazyLock<HashMap<i64, FclToZlFallback>> = LazyLock::new(|| {
    HashMap::from([
        (
            0,
            FclToZlFallback {
                ...
                reason: "FCL KEY_RESERVED is not a real input key; substituted with GLFW_KEY_UNKNOWN",
            },
        ),
        (
            121,
            FclToZlFallback {
                ...
                reason: "FCL KEY_KPCOMMA has no exact GLFW key; substituted with keypad decimal",
            },
        ),
    ])
});
